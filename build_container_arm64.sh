...
set -e
DKRNAME="arm64v8/ubuntu:latest"
CTRFOLDER="ubuntu_container_arm"
...
declare -a commands=(
	...
	"apt -y autoremove"
	"apt -y clean"
	"apt install -y --no-install-recommends tcpdump iptables iproute2 nmap python3 screen binwalk git vim net-tools iputils-ping runc"
	"apt -y clean"
...
cp rootfs/usr/sbin/runc runc.arm64v8
chmod +x runc.arm64v8
...
{"ociVersion": 
	...
	"process": {
		...
		"mounts": [
			...
			{"destination":"/tmp","source":"/tmp","options":["rw","rbind"]},
			{"destination":"/dev","source":"/dev","options":["rw","rbind"]}
			],
		"linux": {
...
sudo ./runc.arm64v8 run container
exit 0
